...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        for (index, element) in stationArray.enumerated() {
            ...
            let trackLayer = CAShapeLayer()
            let circularPath = UIBezierPath(arcCenter: CGPoint.init(x: 0, y: 0), radius: CGFloat(buttonRadius), startAngle: -CGFloat.pi / 2, endAngle: 2 * CGFloat.pi, clockwise: true)
    ...
    func phoneInterruption() {
        ...
        do {
            try theSession.setCategory(AVAudioSession.Category(rawValue: convertFromAVAudioSessionCategory(AVAudioSession.Category.playback)))
    ...
    func goToStation() {
        ...
        if currentButtonNumber == 1 {
            ...
            player = AVQueuePlayer(items: [item1])
            player.play()
            item1.addObserver(self, forKeyPath: "timedMetadata", options: NSKeyValueObservingOptions(), context: nil)
        } else if currentButtonNumber == 2 {
            play = true
            lottieBut?.animationProgress = 0
            player = AVQueuePlayer(items: [item2])
            player.play()
            item2.addObserver(self, forKeyPath: "timedMetadata", options: NSKeyValueObservingOptions(), context: nil)
        } else if currentButtonNumber == 3 {
            play = true
            lottieBut?.animationProgress = 0
            player = AVQueuePlayer(items: [item3])
            player.play()
            item3.addObserver(self, forKeyPath: "timedMetadata", options: NSKeyValueObservingOptions(), context: nil)
        } else if currentButtonNumber == 4 {
            play = true
            lottieBut?.animationProgress = 0
            player = AVQueuePlayer(items: [item4])
            player.play()
            item4.addObserver(self, forKeyPath: "timedMetadata", options: NSKeyValueObservingOptions(), context: nil)
        } else if currentButtonNumber == 5 {
            play = true
            lottieBut?.animationProgress = 0
            player = AVQueuePlayer(items: [item5])
            player.play()
            item5.addObserver(self, forKeyPath: "timedMetadata", options: NSKeyValueObservingOptions(), context: nil)
        } else if currentButtonNumber == 6 {
            play = true
            lottieBut?.animationProgress = 0
            player = AVQueuePlayer(items: [item6])
            player.play()
            item6.addObserver(self, forKeyPath: "timedMetadata", options: NSKeyValueObservingOptions(), context: nil)
        } else if currentButtonNumber == 7 {
            play = true
            lottieBut?.animationProgress = 0
            player = AVQueuePlayer(items: [item7])
            player.play()
            item7.addObserver(self, forKeyPath: "timedMetadata", options: NSKeyValueObservingOptions(), context: nil)
        } else if currentButtonNumber == 8 {
            play = true
            lottieBut?.animationProgress = 0
            player = AVQueuePlayer(items: [item8])
            player.play()
            item8.addObserver(self, forKeyPath: "timedMetadata", options: NSKeyValueObservingOptions(), context: nil)
        }
    }
    ...
    func pathAnim(xPoint:Int, yPoint:Int, radiusSize:CGFloat) {
        ...
        let size: CGFloat = 0
        let circularPath = UIBezierPath(arcCenter: CGPoint.init(x: xPoint, y: yPoint), radius: radiusSize, startAngle: -CGFloat.pi / 2, endAngle: 2 * CGFloat.pi, clockwise: true)
    ...
    private func animatePulsatingLayer(xPoint:Int, yPoint:Int, radiusSize:CGFloat) {
        let midX = self.view.bounds.midX
        let size: CGFloat = 0
        let circularPath = UIBezierPath(arcCenter: CGPoint.init(x: 0, y: 0), radius: CGFloat(buttonRadius)
    ...
    override func observeValue(forKeyPath: String?, of: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
        ...
        if data.timedMetadata != nil {
            for item in data.timedMetadata! {
                currentlyPlaying = (item.value! as? String)!
                songTitle.text = currentlyPlaying
                setupAVAudioSession()
            }
        } else {
            songTitle.text = "Nothing is playing"
        }
    }
    ...
    private func loadStations() -> [Station]?  {
        return NSKeyedUnarchiver.unarchiveObject(withFile: Station.ArchiveURL.path) as? [Station]
    }
}
